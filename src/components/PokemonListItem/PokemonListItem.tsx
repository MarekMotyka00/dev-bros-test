...
function PokemonListItem(props: PokemonListItemProps) {
  ...
  return (
    <Link to={`/pokemon/${pokemonId}`} className='rounded-lg overflow-hidden flex flex-col'>
      <div className='bg-white p-10'>
        <img src={`${baseImgUrl + pokemonId.toString().padStart(3, '0')}.png`} alt='' className='mx-auto' />
      </div>
      <div className='bg-[#FF3E4E] py-6 px-10 text-center text-white flex justify-center items-center capitalize'>{name}</div>
    </Link>
  );
}
